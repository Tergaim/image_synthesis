...
int main(int argc, char *argv[])
{
    ...
    Scene scene;
    scene.camera = Camera(Vec3f(0.0f, 0.f, 0.7f), Vec3f(0.f,0.f,-1.f));
...
bool rayTrace(Image &picture, const Scene &scene, Ray ray, int &indexMesh, int &indexTriangle, float result_glob[4])
{
    ...
    for (int i = 0; i < scene.meshes.size(); i++)
    {
        ...
        for (int j=0; j<m.indexedTriangles().size(); j++)
        {
            ...
            float result[4];
            bool intersect = rayTriangleIntersect(ray, m.vertexPositions()[t[0]], m.vertexPositions()[t[1]], m.vertexPositions()[t[2]], result);
...
Vec3f shade(const Scene& scene, const Vec3f& n, const Vec3f& wi, const int mIndex){
    return scene.meshes[mIndex].material().evaluateColorResponse(n, wi);
}
